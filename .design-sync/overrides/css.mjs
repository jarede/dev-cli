...
// forked from design-sync lib/css.mjs — resolve Vite public-dir absolute url('/fontes/…') @font-face sources
...
export function extractFonts(cssPath, srcDir, { fontsOut, roots }) {
  ...
  for (const m of css.matchAll(/@font-face\s*\{([^}]+)\}/g)) {
    ...
    for (const u of urls) {
      ...
      // Vite serve public/ na raiz: url('/x') = <pkg>/public/x. Tenta os
      // roots com e sem 'public/' antes do fallback relativo ao srcDir.
      let src = resolve(srcDir, u.replace(/^\.\//, ''));
      if (u.startsWith('/')) {
        const rel = u.replace(/^\/+/, '');
        const cand = rootsReal.flatMap((root) => [join(root, 'public', rel), join(root, rel)]);
        src = cand.find((c) => existsSync(c)) ?? src;
      }
